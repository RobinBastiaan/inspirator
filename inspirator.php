<?php
// INDEX(COMPLETE)
// asks the user for input to find program-pages on the wiki
// it is 'complete' because also the style can be found here (all in one file)

// TO DO: extra opties: tips
// TO DO: sinterklaas/kerst
?>
...
<html lang="nl">
	<head>
		...
		<style type="text/css">
			...
			div.mytitle {
				background: #FEE9A0;
				margin: -18px 3px; /* Different on the wiki: margin: 0px 3px; */
				padding: 0px 10px;
			}
			.myfooter {
				font-weight: lighter;
			...
			tr.output > td:first-child {
				width: 155px;
			}
			tr.output > td {
				width: 110px;
			}
			tr.output > td:last-child {
				width: 75px;
			}
			span > span {
				cursor: pointer;
		...
		<div id="mywrap">
			...
			<div class="mybox">
				...
				<div class="mycontent">
					...
					<!-- // Selection for program Type -->
					<div style="float:left;">
						<p class="p">Type programma:</p>
					</div>
					<div style="float:right;">
						<label for="alltype"><input id="alltype" type="checkbox" name="type" checked onClick="mytoggle(this, this.name)" /><i>Alles</i></label><br/>
					</div>
					<table class="table">
						<tr><td class="td" width="150px">
							<label for="creatief"><input id="creatief" type="checkbox" name="type" checked value="creatief" />Creatief</label><br />
							<label for="sport"><input id="sport" type="checkbox" name="type" checked value="sport" />Sport & Spel</label><br />
							<label for="strategisch"><input id="strategisch" type="checkbox" name="type" checked value="strategisch" />Strategisch</label><br />
							<label for="toneel"><input id="toneel" type="checkbox" name="type" checked value="toneel" />Toneel</label>
						</td><td class="td">
							<label for="scoutingtechinieken"><input id="scoutingtechinieken" type="checkbox" name="type" checked value="scoutingtechinieken" />Scoutingtechnieken</label><br />
							<label for="veilig"><input id="veilig" type="checkbox" name="type" checked value="veilig" />Veilig & Gezond</label><br />
							<label for="thema"><input id="thema" type="checkbox" name="type" checked value="thema" />Thema</label><br />
							<label for="korte"><input id="korte" type="checkbox" name="type" value="korte" />Korte spelletjes</label>
						</td></tr>
					</table>
					
					<!-- // Selection for program Loca(tion) -->
					<hr>
					<div style="float:left;">
						<p class="p">Programma locatie:</p>
					</div>
					<div style="float:right;">
						<label for="allloca"><input id="allloca" type="checkbox" name="loca" checked onClick="mytoggle(this, this.name)" /><i>Alles</i></label><br/>
					</div>
					<table class="table">
						<tr>
							<td class="td"><b>Ver weg</b></td>
							<td class="td"><b>Dichtbij</b></td>
						</tr>
						<tr>
							<td class="td" width="150px">
								<label for="bos"><input id="bos" type="checkbox" name="loca" checked value="bos" />Bos</label><br />
								<label for="veld"><input id="veld" type="checkbox" name="loca" checked value="veld" />Veld</label>
							</td>
							<td class="td">
								<label for="clubhuis"><input id="clubhuis" type="checkbox" name="loca" checked value="clubhuis" />Clubhuis</label><br />
								<label for="binnenstad"><input id="binnenstad" type="checkbox" name="loca" checked value="binnenstad" />Binnenstad</label>
							</td>
						</tr>
					</table>
					
					<!-- // Selection for program Date -->
					<hr>
					<p class="p">Minimale herhalingsperiode:</p>
					<input class="myrange" type="range" name="points" min="0" max="48" value="24" step="1" onmousemove="showValue(this.value)">
					<output id="range">0</output>
					
					<!-- // The Search Button Section -->
					<hr>
					<div style="float:left;">
						<label for="ever"><input id="ever" type="checkbox" name="nevers" checked value="ever" /><i>Eens gedaan</i></label> /
						<label for="never"><input id="never" type="checkbox" name="nevers" checked value="never" /><i>Nooit gedaan</i></label><br/>
						<label for="red"><input id="red" type="checkbox" name="red" checked value="red" /><i>Inclusief rode programma's</i></label><br/>
					</div>
					<div style="float:right;">
						<input type="submit" value="Zoek!" onclick="mysearch(1)" />
					</div>
					<br />
				</div>
			</div>
			<div class="mybox">
				<div class="mycontent">					
					<!-- Navigation -->
					<span id="1" onClick="showPrograms(games, found, this.id)">[1]</span>
					<span id="2" onClick="showPrograms(games, found, this.id)">[2]</span>
					<span id="3" onClick="showPrograms(games, found, this.id)">[3]</span>
					<span id="4" onClick="showPrograms(games, found, this.id)">[4]</span>
					<span id="5" onClick="showPrograms(games, found, this.id)">[5]</span>
					<span id="6" onClick="showPrograms(games, found, this.id)">[6]</span>
					<span id="7" onClick="showPrograms(games, found, this.id)">[7]</span>
					<span id="8" onClick="showPrograms(games, found, this.id)">[8]</span>
					<span id="9" onClick="showPrograms(games, found, this.id)">[9]</span>
					<span id="10" onClick="showPrograms(games, found, this.id)">[10]</span>
					<i style="float:right;"><span id="found"></span> resultaten</i>
					
					<hr style="clear:both;"> <!-- Game 1 -->
					<table class="table" id="1eTable">
						<tr><th class="th" colspan="3" id="1eName">1. Geen resultaat.</th></tr>
						<tr class="output">
							<td class="td" id="1eType"><br /></td>
							<td class="td" id="1eLoca"></td>
							<td class="td" id="1eWhen"></td>
						</tr>
					</table>
					
					<hr> <!-- Game 2 -->
					<table class="table" style="clear:both;" id="2eTable">
						<tr><th class="th" colspan="3" id="2eName">2. Geen resultaat.</th></tr>
						<tr class="output">
							<td class="td" id="2eType"><br /></td>
							<td class="td" id="2eLoca"></td>
							<td class="td" id="2eWhen"></td>
						</tr>
					</table>
					
					<hr> <!-- Game 3 -->
					<table class="table" id="3eTable">
						<tr><th class="th" colspan="3" id="3eName">3. Geen resultaat.</th></tr>
						<tr class="output">
							<td class="td" id="3eType"><br /></td>
							<td class="td" id="3eLoca"></td>
							<td class="td" id="3eWhen"></td>
						</tr>
					</table>
					
					<hr> <!-- Game 4 -->
					<table class="table" id="4eTable">
						<tr><th class="th" colspan="3" id="4eName">4. Geen resultaat.</th></tr>
						<tr class="output">
							<td class="td" id="4eType"><br /></td>
							<td class="td" id="4eLoca"></td>
							<td class="td" id="4eWhen"></td>
						</tr>
					</table>
					
					<hr> <!-- Game 5 -->
					<table class="table" id="5eTable">
						<tr><th class="th" colspan="3" id="5eName">5. Geen resultaat.</th></tr>
						<tr class="output">
							<td class="td" id="5eType"><br /></td>
							<td class="td" id="5eLoca"></td>
							<td class="td" id="5eWhen"></td>
						</tr>
					</table>
					
					<hr> <!-- Game 6 -->
					<table class="table" id="6eTable">
						<tr><th class="th" colspan="3" id="6eName">6. Geen resultaat.</th></tr>
						<tr class="output">
							<td class="td" id="6eType"><br /></td>
							<td class="td" id="6eLoca"></td>
							<td class="td" id="6eWhen"></td>
						</tr>
					</table>
			...
			<p class="myfooter" align="right">*Laatst ge&uuml;pdate op 27 augustus 2018, gemaakt door Robin</p>
		</div> <!-- // End Wrap -->
...
<script>/*6*/// Thema // 14
			prog.push (
				...
				// Korte spelletjes // 14
				["<a href='http://franciscus.pbworks.com/w/page/Who's%20the%20man'>Who's the man</a>", "Korte spelletjes", "Clubhuis", "10/2013"],
			...
			function showPrograms(games, found, page) {
				for (var i = 0; i < itemsperpage; i++) {
					if (games[i+((page-1)*itemsperpage)][4] == "Rood") {
						document.getElementById(i+1 + "eName").innerHTML = "<i style='color: #ff0000;'>" + ((i+1)+((page-1)*itemsperpage)) + ". " + games[i+((page-1)*itemsperpage)][0] + "</i>";
					} else {
						document.getElementById(i+1 + "eName").innerHTML = ((i+1)+((page-1)*itemsperpage)) + ". " + games[i+((page-1)*itemsperpage)][0];
					}
					...
					if (games[i+((page-1)*itemsperpage)][2] != "") {
						if (games[i+((page-1)*itemsperpage)][2] == "Bos") {
							document.getElementById(i+1 + "eLoca").innerHTML = "<img src='http://franciscus.pbworks.com/f/bos.png' title='Bos' height='15' width='15'> " + games[i+((page-1)*itemsperpage)][2];
						} else if (games[i+((page-1)*itemsperpage)][2] == "Veld") {
							document.getElementById(i+1 + "eLoca").innerHTML = "<img src='http://franciscus.pbworks.com/f/veld.png' title='Veld' height='15' width='15'> " + games[i+((page-1)*itemsperpage)][2];
						} else if (games[i+((page-1)*itemsperpage)][2] == "Clubhuis") {
							document.getElementById(i+1 + "eLoca").innerHTML = "<img src='http://franciscus.pbworks.com/f/clubhuis.png' title='Clubhuis' height='15' width='15'> " + games[i+((page-1)*itemsperpage)][2];
						} else if (games[i+((page-1)*itemsperpage)][2] == "Binnenstad") {
							document.getElementById(i+1 + "eLoca").innerHTML = "<img src='http://franciscus.pbworks.com/f/binnenstad.png' title='Binnenstad' height='15' width='15'> " + games[i+((page-1)*itemsperpage)][2];
						} else { // standard value
							document.getElementById(i+1 + "eLoca").innerHTML = "<img src='http://franciscus.pbworks.com/f/locatie.png' title='Locatie' height='15' width='20'> " + games[i+((page-1)*itemsperpage)][2];
						}
					...
					if (games[i+((page-1)*itemsperpage)][3] == "0/0") {
						document.getElementById(i+1 + "eWhen").innerHTML = "<img src='http://franciscus.pbworks.com/f/wanneer.png' title='Wanneer' height='15' width='15'> -- / ----";
					} else if (games[i+((page-1)*itemsperpage)][3] != "") {
						document.getElementById(i+1 + "eWhen").innerHTML = "<img src='http://franciscus.pbworks.com/f/wanneer.png' title='Wanneer' height='15' width='15'> " + games[i+((page-1)*itemsperpage)][3];
					} else { // no program = empty
				...
				for (var j = 1; j <= 10; j++) { // max 10 pagemarkers
...
<script>/*8*/// search for the programs
			function mysearch(page) {
				// new type-input
				var type = new Array();
				if (document.querySelector('input[value="creatief"]').checked) { type.push("Creatief"); }
				if (document.querySelector('input[value="sport"]').checked) { type.push("Sport & Spel"); }
				if (document.querySelector('input[value="strategisch"]').checked) { type.push("Strategisch"); }
				if (document.querySelector('input[value="toneel"]').checked) { type.push("Toneel"); }
				if (document.querySelector('input[value="scoutingtechinieken"]').checked) { type.push("Uitdagende Scoutingtechnieken"); }
				if (document.querySelector('input[value="veilig"]').checked) { type.push("Veilig & Gezond"); }
				if (document.querySelector('input[value="thema"]').checked) { type.push("Thema"); }
				if (document.querySelector('input[value="korte"]').checked) { type.push("Korte spelletjes"); }

				// new loca-input
				var loca = new Array();
				if (document.querySelector('input[value="bos"]').checked) { loca.push("Bos"); }
				if (document.querySelector('input[value="veld"]').checked) { loca.push("Veld"); }
				if (document.querySelector('input[value="clubhuis"]').checked) { loca.push("Clubhuis"); }
				if (document.querySelector('input[value="binnenstad"]').checked) { loca.push("Binnenstad"); }
				
				// new special option-input
				var ever = false;
				var never = false;
				var red = false;
				if (document.querySelector('input[value="ever"]').checked) { ever = true; }
				if (document.querySelector('input[value="never"]').checked) { never = true; }
				if (document.querySelector('input[value="red"]').checked) { red = true; }
				
				// algorithm to set values
				var rank = 1;
				found = 0;
				games = new Array(); // reset game values
				
				for (var i = 0; i < prog.length; i++) { // cycle through all prog
					for (var j = 0; j < type.length; j++) { // cycle through prog-type checked
						for (var k = 0; k < loca.length; k++) { // cycle through prog-loca checked
							if (myinclude(prog[i][1],type[j])) { // check if the prog has the right type
								if (myinclude(prog[i][2],loca[k])) { // check if the prog has the right loca
									if (never == true && ever == true) { // check if user wants to see ever&never done prog's
										if (red == true) { // check if user wants to include red prog's
											if (mycalc_diff(prog[i][3]) >= when) { // check if the prog has right date
												games.push(eval("prog[" + i + "]")); // all prog's
												rank++; found++;
											}
										} else { // all not-red prog's and right date
											if ((prog[i][4] != "Rood") && (mycalc_diff(prog[i][3]) >= when)) { // check if the prog is not red and right date
										...
										if ((prog[i][3] != "0/0") && (mycalc_diff(prog[i][3]) >= when)) { // check if the prog has ever been done and right date
											games.push(eval("prog[" + i + "]"));
											rank++; found++;
				...
				games.sort(function(a,b) { return parseFloat(mycalc_diff(b[3])) - parseFloat(mycalc_diff(a[3])) } );
...
<script>/*9*/// check if a values is in an array
			...
			function mycalc_diff(prog_date) {
				// get today value
				var today = new Date();
				var mm = today.getMonth()+1; //January is 0!
				var yyyy = today.getFullYear();
				
				var when_res = prog_date.split("/");
				months = ((yyyy - when_res[1]) * 12) + (mm - when_res[0]); // calculate difference in months
				return months;
			}
			
			// initial function calls
			window.addEventListener("onload", showValue(repeat), false); // initial slider-range
			window.addEventListener("onload", mysearch(1), false); // initial programs
		//-->
		</script>
	</body>
</html>
